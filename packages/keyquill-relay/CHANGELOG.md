...
- 02c76e3: Mobile (`keyquill-mobile`) gains an audit ledger that mirrors the
...
- 0aa0d9f: Mobile (`keyquill-mobile`) gains an audit ledger that mirrors the
  extension's `background/ledger.ts` API, persisting per-request history
  to `@capacitor/preferences` (NSUserDefaults on iOS, SharedPreferences
  on Android).
  - `appendEntry`, `queryByProvider`, `getMonthSpend`, `getDailySpend`,
...
- d2fb429: Mobile (`keyquill-mobile`) gains stable error codes and English /
...
- abe7a6e: Mobile (`keyquill-mobile`) gains a TS-side resolver and a
...
- 3e0be0e: Mobile (`keyquill-mobile`) gains the broker `KeyPolicy` type surface
...
- 8ccf364: Mobile (`keyquill-mobile`) gains the broker type surface — `ModelCatalog`,
