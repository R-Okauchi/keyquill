...
2. **`02-demo-ready.png`** — The [live demo](https://r-okauchi.github.io/llmvault/demo/) after the extension is loaded, at the "Extension ready. Click Connect to grant this page access." state.
...
1. Build & load the extension:
   ```bash
   pnpm --filter llmvault-extension build
   ```
   Then: `chrome://extensions` → Developer mode → Load unpacked → `packages/llmvault-extension/dist-chrome/`.
