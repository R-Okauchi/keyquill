...
Drop-in assets for submitting `llmvault-extension` to the Chrome Web Store and Firefox AMO.
...
2. Store listing → **Name** (per language): `LLMVault` (both).
...
12. Privacy practices → **Privacy policy URL**: `https://r-okauchi.github.io/llmvault/privacy-policy`
...
5. **Privacy Policy URL**: `https://r-okauchi.github.io/llmvault/privacy-policy`.
6. **License**: MIT (from dropdown).
7. **Support email / website**: GitHub issues URL.
8. **Screenshots**: reuse the same PNGs from `screenshots/`.
9. **Source code**: if the reviewer asks, link the GitHub repo. If they still require a zip, create one from the `llmvault-extension-v<version>` tag.

## Regenerating the promo tile

```bash
cd /Users/okauchiryota/main_desk/personal/llmvault
npx --yes @resvg/resvg-js-cli --fit-width 440 \
  packages/llmvault-extension/submission/promo/tile-440x280.svg \
  packages/llmvault-extension/submission/promo/tile-440x280.png
```
...
1. Build + load the extension unpacked from `packages/llmvault-extension/dist-chrome/`.
2. Register a test API key via the extension popup.
3. Visit <https://r-okauchi.github.io/llmvault/demo/>.
