 ...
 *
 * Detection: content script injects <meta name="keyquill-extension-id">.
...
/**
 * Check if the Keyquill extension content script has injected its meta tag.
 */
...
export function detectExtensionId(knownIds: string[]): string | null {
  if (typeof document !== "undefined") {
    const meta = document.querySelector('meta[name="keyquill-extension-id"]');
    if (meta) {
 ...
 *
 * Web page posts: { type: "keyquill-request", id, payload }
 * Content script relays to background and posts back:
 *   { type: "keyquill-response", id, payload }
 */
export function sendExtensionMessage<T>(
  ...
  return new Promise<T | null>((resolve) => {
    ...
    function handler(event: MessageEvent) {
      ...
      if (event.data?.type !== "keyquill-response" || event.data.id !== id) return;
      clearTimeout(timer);
      window.removeEventListener("message", handler);
    ...
    window.postMessage({ type: "keyquill-request", id, payload: message }, location.origin);
  });
}
 ...
 *
 * Web page posts: { type: "keyquill-stream-open", id, payload }
 * Content script opens a real Port and relays events back as:
 *   { type: "keyquill-stream-event", id, payload }
 *   { type: "keyquill-stream-close", id }
 */
export function connectToExtension(_extensionId: string, _portName: string): ExtensionPort | null {
  ...
  function handler(event: MessageEvent) {
    ...
    if (data.type === "keyquill-stream-event") {
      for (const cb of messageListeners) cb(data.payload);
    } else if (data.type === "keyquill-stream-close") {
      connected = false;
      window.removeEventListener("message", handler);
  ...
  const port: ExtensionPort = {
    ...
    postMessage(message: unknown) {
      if (!connected) return;
      // The initial message is sent via keyquill-stream-open.
      // Subsequent messages go through a separate channel if needed.
      window.postMessage({ type: "keyquill-stream-open", id, payload: message }, location.origin);
    },
    onMessage: {
