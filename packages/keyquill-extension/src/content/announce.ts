...
function injectMeta(): void {
  ...
  meta.name = "keyquill-extension-id";
...
window.dispatchEvent(
  new CustomEvent("keyquill-available", {
 ...
 * Web page sends: { type: "keyquill-request", id, payload }
 * Content script forwards payload to background via runtime.sendMessage,
 * then posts back: { type: "keyquill-response", id, payload }
 */
window.addEventListener("message", (event: MessageEvent) => {
  if (event.source !== window) return;
  if (event.origin !== location.origin) return;
  const data = event.data;
  if (!data || data.type !== "keyquill-request") return;
  ...
  if (payload?.type && BLOCKED_TYPES.has(payload.type)) {
    window.postMessage(
      {
        type: "keyquill-response",
        id,
        payload: {
          type: "error",
          code: "BLOCKED",
          message: "This operation is only available from the Keyquill extension popup.",
        },
      },
      location.origin,
    );
  ...
  ext.runtime.sendMessage(payload, (response: unknown) => {
    window.postMessage(
      { type: "keyquill-response", id, payload: response ?? null },
      location.origin,
    );
  });
});
...
/**
 * Streaming relay.
 * Web page sends: { type: "keyquill-stream-open", id, payload }
 * Content script opens a Port to background, forwards the payload,
 * then relays all messages back as: { type: "keyquill-stream-event", id, payload }
 * When done/disconnected: { type: "keyquill-stream-close", id }
 */
window.addEventListener("message", (event: MessageEvent) => {
  if (event.source !== window) return;
  if (event.origin !== location.origin) return;
  const data = event.data;
  if (!data || data.type !== "keyquill-stream-open") return;

  const { id, payload } = data;

  const port = ext.runtime.connect({ name: "keyquill-chat" });

  port.onMessage.addListener((msg: unknown) => {
    window.postMessage({ type: "keyquill-stream-event", id, payload: msg }, location.origin);
  });

  port.onDisconnect.addListener(() => {
    window.postMessage({ type: "keyquill-stream-close", id }, location.origin);
  });
